...
{
    for (arc::Color& color : m_colors) {
        color = m_defaultBgColor;
    }
}

void DrawingBoard::setColor(int x, int y, arc::Color color)
{
    if (!isInRange(x, y))
        return;
...
void DrawingBoard::renderLine(int x, int y, int len, const arc::Color& color, RotationDir direction)
{
    switch (direction) {
...
void DrawingBoard::renderLine(int x, int y, const arc::Color& color, RotationDir direction)
{
    if (!isInRange(x, y))
        return;
...
void DrawingBoard::renderVectorShape(int x, int y, const VectorShape& vectorShape)
{
    ...
    for (const Vector& vector : vectorShape.vectors()) {
        x += vector.x;
        y += vector.y;
        setColor(x, y, arc::colors(vectorShape.color()));
        // loggerPtr->log(DEBUG) << " setColor [" << x << ", " << y << "]";
    }
}
...
Solver::Rules Solver::gridDiff(const Grid& input, const Grid& output)
{
    Solver::Rules ret;
    return ret;
}

Solver::Code Solver::processRules(const std::vector<Rules>& rules)
{
    Solver::Code ret;
    return ret;
}

DrawingBoard Solver::applyCode(const Solver::Grid& input, const Solver::Code& code)
{
